    ...
    while iteration < opt.iterations + 1:
        for viewpoint_batch in training_generator:
            for viewpoint_cam in viewpoint_batch:
                ...
                def safe_update_sh_degree(gaussians, manager, iteration):
                    """安全地更新球谐函数度数"""
                    if iteration % 1000 == 0:
                        current_sh_degree = gaussians.active_sh_degree
                        max_sh_degree = gaussians.max_sh_degree

                        if current_sh_degree < max_sh_degree:
                            # 获取当前特征维度
                            current_dc_shape = gaussians._features_dc.shape
                            current_rest_shape = gaussians._features_rest.shape if hasattr(gaussians,
                                                                                           '_features_rest') else None

                            # 调用原始的oneupSHdegree
                            gaussians.oneupSHdegree()

                            # 检查维度是否改变
                            new_sh_degree = gaussians.active_sh_degree
                            new_sh_dim = (new_sh_degree + 1) ** 2

                            # 如果维度改变了，需要调整特征张量
                            if current_rest_shape is not None:
                                expected_rest_dim = new_sh_dim - 1
                                current_rest_dim = current_rest_shape[-1]

                                if expected_rest_dim != current_rest_dim:
                                    print(f"  Adjusting SH dimensions: {current_rest_dim} -> {expected_rest_dim}")

                                    with torch.no_grad():
                                        # 创建新的rest特征张量
                                        n_points = gaussians._features_rest.shape[0]
                                        new_features_rest = torch.zeros(
                                            (n_points, 3, expected_rest_dim),
                                            device='cuda',
                                            dtype=torch.float32
                                        )

                                        # 复制现有的系数
                                        min_dim = min(current_rest_dim, expected_rest_dim)
                                        new_features_rest[:, :, :min_dim] = gaussians._features_rest[:, :, :min_dim]

                                        # 更新特征张量
                                        gaussians._features_rest = torch.nn.Parameter(new_features_rest)

                                        # 重新设置优化器参数
                                        if hasattr(gaussians, 'optimizer'):
                                            # 找到_features_rest在优化器中的参数组
                                            for group in gaussians.optimizer.param_groups:
                                                params = group['params']
                                                for i, param in enumerate(params):
                                                    if param is gaussians._features_rest:
                                                        # 替换参数
                                                        params[i] = gaussians._features_rest
                                                        break
