...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
                 resolution_scales=[1.0], create_from_hier=False):
        """b
        ...
        self.train_cameras = {}
        self.test_cameras = {}
        self.train_cameras_temp = {}

        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.alpha_masks, args.depths,
        ...
        for resolution_scale in resolution_scales:
            print("Making Training Dataset")  # 针对每个分辨率缩放比例，创建训练相机数据集    # CameraDataset 是一个自定义类，用来封装相机数据
            self.train_cameras[resolution_scale] = CameraDataset(scene_info.train_cameras, args, resolution_scale,
                                                                 False)
            self.train_cameras_temp[resolution_scale] = cameraList_from_camInfos(scene_info.train_cameras,
            ...
            camera_centers = []
            center_rays = []
            for id, cur_cam in enumerate(self.train_cameras_temp[resolution_scale]):
                self.world_view_transforms.append(cur_cam.world_view_transform)  # 提取变换矩阵
                camera_centers.append(cur_cam.camera_center)  # 提取相机中心
            ...
            with open(os.path.join(self.model_path, "multi_view.json"), 'w') as file:
                for id, cur_cam in enumerate(self.train_cameras_temp[resolution_scale]):
                    ...
                    for index in sorted_indices[:multi_view_num]:
                        ...
                        cur_cam.nearest_names.append(self.train_cameras_temp[resolution_scale][index].image_name)
                        json_d["nearest_name"].append(self.train_cameras_temp[resolution_scale][index].image_name)

                        # ## 在这里添加
                        # if self.train_cameras[1.0].image_name == cur_cam.nearest_names:
                        #     self.train_cameras.nearest_name = cur_cam.nearest_names
                        #     self.train_cameras.nearest_id = cur_cam.nearest_id

                    json_str = json.dumps(json_d, separators=(',', ':'))
                    file.write(json_str)
                    file.write('\n')

        ####  写入文本的时候在训练视角中添加对应的nearest_id和nearest_names属性

        if self.loaded_iter:  # 检查是否已经加载了特定的迭代信息
            self.gaussians.load_ply(
                os.path.join(self.model_path,  # 路径结构为模型路径 -> point_cloud 文件夹 -> 对应迭代次数的文件夹 -> point_cloud.ply 文件
                             "point_cloud",
                             "iteration_" + str(self.loaded_iter),
                             "point_cloud.ply"))
        elif args.pretrained:  # 若没有加载迭代信息，但指定了预训练模型文件
            self.gaussians.create_from_pt(args.pretrained,
                                          self.cameras_extent)  # self.cameras_extent 是相机的范围，用于归一化等操作  # 从层次结构文件创建高斯点云   # args.hierarchy 是层次结构文件的路径
        elif create_from_hier:  # 若既没有加载迭代信息，也没有指定预训练模型文件，但设置了从层次结构创建
            self.gaussians.create_from_hier(args.hierarchy, self.cameras_extent,
                                            args.scaffold_file)  # self.cameras_extent 是相机的范围   # args.scaffold_file 是脚手架文件的路径，可能用于辅助构建点云
        else:
    ...
    def getTrainCameras(self, scale=1.0, multi_view=False):
        if multi_view == False:
            ...
            return self.train_cameras_temp[scale]
