...
def loadCam(args, id, cam_info, resolution_scale, is_test_dataset, loadCam_scene=False):
    ...
    return Camera(resolution, colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  nearest_id=cam_info.nearest_id, nearest_names=cam_info.nearest_names)

def cameraList_from_camInfos(cam_infos, resolution_scale, args, is_test_dataset):
    camera_list = []

    for id, c in enumerate(cam_infos):
        camera_list.append(loadCam(args, id, c, resolution_scale, is_test_dataset, True))
...
class CameraDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        # index_nearest = random.choice(info.nearest_id)
        # info_nearest = self.list_cam_infos[index_nearest]
        X = loadCam(self.args, index, info, self.resolution_scales, self.is_test)
        # nearest_cam = loadCam(self.args, index_nearest, info_nearest, self.resolution_scales, self.is_test)
        # return X, nearest_cam
        return X
