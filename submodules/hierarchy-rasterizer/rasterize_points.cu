	...
	const bool do_depth)
{
  ...
  torch::Tensor out_observe = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));  // 初始化
  ...
  return std::make_tuple(rendered, out_color, radii, out_observe, out_all_map, out_plane_depth, geomBuffer, binningBuffer, imgBuffer, out_invdepth);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dall_map = torch::zeros({fullP, NUM_ALL_MAP}, means3D.options());
//   torch::Tensor dL_dall_map = torch::zeros({P, NUM_ALL_MAP}, means3D.options());
  ...
  {
//         // 将radii复制到CPU以进行主机访问
//         torch::Tensor radii_cpu = radii.cpu();
//         const int* radii_data = radii_cpu.data<int>();
//         printf("Radii values (on CPU):\n");
//         for (int i = 0; i < radii_cpu.size(0); i += 500) {
//             printf("radii[%d]: %d\n", i, radii_data[i]);
//         }
// 	  // 打印 radii 的内容
...
//       printf("radii: %d\n", radii.data<int>());
	  ...
	  CudaRasterizer::Rasterizer::backward(P, degree, M, R,  // 反向传播
	  background.contiguous().data<float>(),
