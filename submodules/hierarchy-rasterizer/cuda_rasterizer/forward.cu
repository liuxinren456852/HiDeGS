...
// 	constexpr float h_var = 0.3f;  // kernel size
	constexpr float h_var = 0.1f;
	...
	const bool render_geo)
{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (render_geo) {
				for (int ch = 0; ch < ALL_MAP; ch++)
					All_map[ch] += all_map[collected_id[j] * ALL_MAP + ch] * alpha * T;  // 根据其混合权重（alpha）和累积透明度（T）累加到当前像素的几何信息（All_map）中
			}
	...
	if (inside)   // 如果当前像素是有效的
	{
		...
		if (render_geo) {
			...
			out_plane_depth[pix_id] = All_map[4] / -(All_map[0] * ray.x + All_map[1] * ray.y + All_map[2] + 1.0e-8);  // 计算并写入平面深度
		}
	}
}
...
void FORWARD::render(
	...
	const bool render_geo)
{
	renderCUDA<NUM_CHANNELS, NUM_ALL_MAP> << <grid, block, 0, stream >> > (
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
